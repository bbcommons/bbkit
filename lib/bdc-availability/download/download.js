...
async function download(options) {
  ...
  try {
    // TODO this rejects if there's a single error; meanwhile, files are still 
    // downloading. should we handle more errors to make this collection of
    // promises more synchronous? actually maybe it's good in that it sort of
    // short-circuits the for-loop if there's an error detected (e.g. rate-
    // limiting)?
    await downloadFiles(outDirPath, urlsAndFileNames);
  } catch (e) {
    console.error(`Error downloading files: ${e}`);
  }

  // TODO this fires too early when there's an error; see comment above.
...
//   filingType: 'fixed',
//   tech: '60',
//   outputDir: '~/Downloads/bbkit-test',
//   // format: 'shp',
//   // filingDate: '',
// });
